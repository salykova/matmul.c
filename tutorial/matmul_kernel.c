...
void kernel_16x6(float* A, float* B, float* C, const int M, const int N, const int K) {
  ...
  for (int p = 0; p < K; p++) {
    ...
    b_packFloat8 = _mm256_broadcast_ss(&B[1 * K + p]);
    ...
    b_packFloat8 = _mm256_broadcast_ss(&B[2 * K + p]);
    ...
    b_packFloat8 = _mm256_broadcast_ss(&B[3 * K + p]);
    ...
    b_packFloat8 = _mm256_broadcast_ss(&B[4 * K + p]);
    ...
    b_packFloat8 = _mm256_broadcast_ss(&B[5 * K + p]);
